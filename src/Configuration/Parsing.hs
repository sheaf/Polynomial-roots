...
runParse parser name str = liftM (first show) (runParserT parser () name str)
...
pEnumerated = pByShow [ minBound .. maxBound ]
...
pComplex :: forall a u m
         .  (Monad m, Num a, RealFloat a)
         => ParsecT String u m a -> ParsecT String u m (Complex a)
pComplex p = do s1 <- option id plusOrMinus
                z1 <- q
                z2 <- option (0 :+ 0) q'
                return $ s1 z1 + z2
                <?> "complex coordinates"
                    where q = try $ pString "i" *>
                                          choice [ lookAhead ( plusOrMinus @a ) *> return (0:+1)
                                                 , (0 :+) <$> option 1 p]
...
plusOrMinus :: forall a u m. (Monad m, Num a) => ParsecT String u m (a -> a)
plusOrMinus =
  do s <- pStrings ["+","-"]
     return $ case s of
                   "-" -> negate
...
pByShow :: forall u m a. (Monad m, Show a) => [a] -> ParsecT String u m a
pByShow xs = choice $ map byShow xs
  where
    byShow :: a -> ParsecT String u m a
    byShow x = try (pString (show x) *> return x)
...
between' :: Stream s m t => ParsecT s u m close -> ParsecT s u m a -> ParsecT s u m a

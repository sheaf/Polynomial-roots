...
ifsCheatCounts :: (Coefficient a) => [Complex Double] -> [Polynomial a] 
               -> Config v c a -> [IFSPlot a]
ifsCheatCounts scales pols (Config _ res d c w _) = map IFSPlot points
...
getScales :: (Coefficient a) => Config v c a -> [Complex Double]
...
ifsPoints :: (Coefficient a) => Config v c a -> [IFSPlot a]
